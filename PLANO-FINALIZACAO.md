...
| I9.12 (parcial) | **metade executora**: `emitir_hook_memoria_fn` emite `mem_adquirir` e `mem_devolver` nos dois hooks a partir de fonte única; o prepare adquire ANTES de desligar o display e de a GPU ser destacada, e falhar ali aborta o start com o desktop de pé; o release devolve e **nunca aborta**, contando a falha para que GPU e display sejam restaurados do mesmo jeito. Estado persistente em `/var/lib/vm-passthrough/<vm>.memoria`, modo 0600, publicado por rename, gravado ANTES da escrita no pool e com `BOOT_ID` para reconciliar estado de outro boot sem tocar no pool | aritmética em Bash puro dentro do hook por exigência de I9-D8 (o hook roda com o projeto apagado); o core continua sendo o planejador, e a etapa 14 assa `MEMORIA_MODO`, `MEM_PAGE_KB` e `MEM_PAGES_NEEDED` derivados de `resources-plan`, sem repetir a tabela de modos |
| I9.12 (parcial) | **contrato da etapa 17 substituído**: o critério passou a ser a POLÍTICA e não a reserva estática. No perfil retornável, a ausência das três chaves de boot virou pós-condição de SUCESSO e o XML tem de NÃO exigir página grande; no perfil legado `hugetlb-1g-boot` o contrato antigo continua e ganha o aviso explícito de não ser retornável; `MEMORIA_MODO` vazio é relatado como decisão pendente, nunca assumido em silêncio | medido no host depois da migração: onde o verificador dizia `Pool de HugePages divergente` e `XML ... precisa existir exatamente uma vez`, agora diz `Nenhuma reserva de HugePages na cmdline deste boot`, `XML ... NÃO exige HugePages: é o que torna a VM iniciável com memória comum` e `Política 'normal': a VM usa memória comum, que volta ao host quando o QEMU termina`. Restam como pendência honesta a decisão de `MEMORIA_MODO` e o resíduo `HUGEPAGES_1G=22` no conf |
