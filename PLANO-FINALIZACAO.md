...
| I9.12 (retomada, tentativa 1) | 2026-09-05 | `af07725` + working tree | em curso; baseline registrado antes da primeira mudança; decisões I9.12-D8 a D14 gravadas na fase I9; **passo 1** `libexec/passthrough_core/resources.py`, `tests/python/test_resources.py`; **passo 2** `config.py`, `lib/shell/config.sh`, `tests/python/test_config.py`, `tests/test-i4-config.sh`, `tests/test-i0-characterization.sh`, `tests/test-inventario-redetectar.sh`, `tests/lib/{i1-guard,mutator}-harness.sh`, `tests/i0/traceability.tsv`; **passo 3** `cpu.py`, `tests/python/test_cpu.py`, `lib/shell/probes.sh`, `etapas/02-detectar-config.sh`, `etapas/52-cpu-pinning-hugepages.sh`, `tests/test-i5-cpu-boot.sh`; **passos 4 e 5** `domain_xml.py`, `tests/python/test_domain_xml.py`, `lib/shell/libvirt.sh`, `tests/test-cpu-hugepages.sh`, `tests/test-i3-domain-transactions.sh`; **passos 6 e 7** `etapas/02-detectar-config.sh`, `etapas/52-cpu-pinning-hugepages.sh`, `menu.sh`; **passo 8** `etapas/50-hooks-gpu-hd1.sh`, `tests/test-i912-memoria-hooks.sh`, `tests/test-i9-hooks-isolados.sh`; **passo 9** `passthrough.conf.example`, `README.md`, `Guia-QEMU-Passthrough.md`, `troubleshooting.md`, `util/diagnostico.sh`, `tests/test-i0-characterization.sh`, `.kiro/steering/{product,tech}.md`, `.kiro/specs/boot-initramfs-multidistro/*.md`; **passo 10** novo `tests/test-i912-etapas-17-18.sh`, `tests/manifests/i9-files.txt`; **passo 11** `tests/test-gpu-udev-loop.sh` e `tests/test-runtime-lifecycles.sh` (fixtures de conf que renderizam hooks passaram a declarar `MEMORIA_MODO`) | baseline VERDE: `run_tests.py` **1146 casos OK**; `test-python-core.sh` rc 0 (com `env -u FORCE_COLOR`); `test-i912-memoria-hooks.sh` **87 casos** rc 0; `test-cpu-hugepages.sh`, `test-i5-cpu-boot.sh`, `test-i4-config.sh`, `test-i0-characterization.sh`, `test-inventario-redetectar.sh`, `test-i9-hooks-isolados.sh` rc 0; `check-phase-manifest.sh I9` 161 caminhos; `test-i1-safety-envelope.sh` 30 mutadores + 23 seleções rc 0 | **EM CURSO** | nada implementado ainda nesta linha; o Gate I9 continua sem veredito observado | esta seção e a fase I9 | passos 1 a 3 FEITOS. 1: `MODES` com três modos, `returnable` sempre 1, ramo do perfil estático removido, `PlanBootModeTests` virou `PlanPoolPreexistenteTests` com o oráculo anterior citado. 2: `HUGEPAGES_1G` fora do schema (46 chaves) e dentro de `DEPRECATED_KEYS`/`CHAVES_CONF_DEPRECIADAS`, alternância do `grep` da migração montada a partir do array, redetecção zera `MEMORIA_MODO` no lugar dela, duas linhas da rastreabilidade reescritas. 3: `memory_plan` sem `hugepages_1g` (entrada e saída), `plano_memoria_vm` com dois argumentos, dois chamadores ajustados. Suítes: **1148 casos** Python OK; `test-i4-config.sh`, `test-i0-characterization.sh`, `test-inventario-redetectar.sh`, `test-i5-cpu-boot.sh` rc 0. 4 e 5: `cpu-pinning` com `memory_mode` obrigatório em três valores, helper `_drop_hugepages` compartilhado com `remove-hugepages`, `validate_cpu_pinning` com `nao|2m|1g|ignorar` comparando bytes, fachada shell com nono argumento, fixtures Bash em `hugetlb-2m` e `normal` com recusa cruzada. Suítes: **1158 casos** Python OK; `check-python-boundary.py`, `test-cpu-hugepages.sh`, `test-i3-domain-transactions.sh`, `test-i0-characterization.sh` rc 0. 6: etapa 3 pergunta `MEMORIA_MODO` em três opções sem padrão implícito e não grava mais `HUGEPAGES_1G`; `--verificar` da etapa 3 exige a chave. 7: etapa 17 aplica **só XML** (`param_hugepages`, `hugepages_estado_exato`, `_v_inteiro_hugepages` e `validar_suporte_1g` removidos; `validar_suporte_modo` por modo; recusa fail-closed se o boot ainda reservar HugePages; recusa estrutural do plano bloqueia e transitória só avisa), menu sem campo de reboot. Suítes: `test-inventario-redetectar.sh`, `test-ubuntu-audit-regressions.sh`, `test-cpu-hugepages.sh` (ordem `candidate,topologia,define`, `kernel_param_add` e `pedir_reboot` provados nunca chamados), `test-i5-cpu-boot.sh`, `test-i9-hooks-isolados.sh` rc 0; `test-i1-safety-envelope.sh` 30+23 rc 0. 8: etapa 14 exige `MEMORIA_MODO` no `exigir_conf`, `emitir_hook_memoria_fn` e `memoria_plano_resolver` sem `:-normal`, e o `--verificar` compara a linha `MEMORIA_MODO=` do prepare INSTALADO com o conf (divergência vira pendência, hook ilegível vira indeterminado). Caracterização invertida do corpus feita junto: as duas linhas `1g-boot-*` viraram `plano=0, recusa|recusa` com o oráculo anterior citado, e o par A13d trocou para `normal`. Suítes: `test-i912-memoria-hooks.sh` **89 casos** (eram 87), `test-i9-hooks-isolados.sh`, `test-i9-airlock-verify.sh`, `test-ubuntu-audit-regressions.sh`, `test-i1-safety-envelope.sh` rc 0; **1158 casos** Python OK. 9: exemplo sem `HUGEPAGES_1G` e sem `hugetlb-1g-boot` (hash novo `7144e90e…`, justificativa registrada no baseline I0 citando o anterior `77323888…`), README/Guia/troubleshooting descrevendo o perfil retornável e o diagnóstico por `hooks.log`/`<VM>.memoria`, `util/diagnostico.sh` provando baseline com a VM parada, Kiro sincronizado (regra 20). 10: suíte dirigida nova `tests/test-i912-etapas-17-18.sh` com **26 casos** (etapa 17: os três modos aprovando o próprio XML, recusa cruzada nos dois sentidos, política indecisa, `hugetlb-1g-boot` como erro de configuração, chave de boot recusada em qualquer modo, regressão de `HUGEPAGES_1G` que não muda o rc, e a asserção estática de que `kernel_param_add` sumiu e `kernel_param_del`/`pedir_reboot` só existem em `desfazer`; etapa 18: os quatro casos da pendência 1, mais a prova de que a confirmação digitada vem antes da escrita de boot); manifesto I9 com **162** caminhos. 11: validação hermética completa VERDE. `bash -n` em todos os shell; `git diff --check`; **1158 casos** Python; `test-python-core.sh`; `check-python-boundary.py`; `check-waivers-matrix.py` (4 políticas, 2 chaves); `check-phase-manifest.sh I9` **162** caminhos; as **25** suítes `tests/test-*.sh` do laço rc 0; `test-i1-safety-envelope.sh` 30+23 em 6 perfis duas vezes; `test-atualizar-host-validation.sh` 29 cenários. Falta o passo 12: Gate I9 canônico com veredito observado. núcleo sem `hugetlb-1g-boot`, `HUGEPAGES_1G` depreciada, XML por tamanho de página, etapas 3/14/17, suíte nova das etapas 17 e 18, docs, gate |
