...
> **Status:** I0 e I1 concluídas e reverificadas em 16/08/2026; I2 a I5 em 17/08/2026; I6 em 23/08/2026; I7 e I8 em 28/08/2026 (Gates I7 e I8 aprovados). **I9 CONCLUÍDA em 05/09/2026**, com o Gate I9 reexecutado e o veredito `OK: Gate I9 concluído sem mascarar status` OBSERVADO. As tarefas I9.1 a I9.13 estão feitas e provadas. I9.12 (REQ-VM-RESOURCE-LIFECYCLE) fechou na retomada de 05/09/2026: o perfil de reserva de HugePages no boot foi REMOVIDO do projeto (I9.12-D8 a D14), a etapa 17 aplica somente XML dirigida por `MEMORIA_MODO`, a etapa 3 pergunta a política e a etapa 14 recusa assar política não decidida. Resta de I9.12 apenas a qualificação em hardware, que é de I13, e a sequência do operador neste host. A infraestrutura de I9B construída em 02/09/2026 foi perdida sem commit. **Próxima fase: I9B.** Fases I9B a I14 pendentes. **Última auditoria plano contra código: 03/09/2026, seção 1.6**
...
3. Leia a última linha preenchida da seção 12 (registro de execução): ela nomeia a fase ativa e a próxima ação. Em 05/09/2026 a fase I9 foi CONCLUÍDA com o Gate I9 observado, e a fase ativa passou a ser **I9B**, que precisa ser reconstruída na árvore a partir de I9B.1.
...
| 11 | I9 | Modularização Bash e requisitos P1 restantes | `CONCLUÍDA` 2026-09-05 | `lib/shell/*`, REQ-WINDOWS-STATE, REQ-AIRLOCK-VERIFY, REQ-VM-RESOURCE-LIFECYCLE |
...
**Fases:** contrato e implementação em **I9.12** (fase I9 reaberta por este requisito em 02/09/2026); baterias simuladas em I10 e I12; aceite operacional em I13. Estado: `CÓDIGO COMPLETO` — migração deste host concluída e baseline retornável provado em 03/09/2026; ciclo de aquisição e devolução implementado nos hooks e coberto por bateria com sysfs simulado; e, na retomada de 05/09/2026, o requisito fechado no código: perfil de reserva estática REMOVIDO do catálogo (I9.12-D8), `HUGEPAGES_1G` depreciada (D9), etapa 17 aplicando somente XML e nunca mais gravando parâmetro de boot (D10), página declarada por modo no XML (D11), etapa 3 perguntando a política (D12) e etapa 14 recusando assar política não decidida (D13). A etapa 18 recusa isolamento persistente desde `10f5e52` e agora **tem teste dirigido** (`tests/test-i912-etapas-17-18.sh`). **Falta** apenas a qualificação em hardware, que é de I13.
...
**A inversão que o requisito impôs, e que foi concluída em 05/09/2026.** O parágrafo abaixo descreve o PASSADO, e fica registrado porque explica o tamanho da migração: até `af07725` a etapa 17 tratava reserva estática de 1 GiB como o contrato, e o `--verificar` dela exige `HugePages_Total` exatamente igual a `HUGEPAGES_1G` — ou seja, **o estado que este requisito considera defeito é hoje a pós-condição de sucesso**. A migração, portanto, não é acrescentar um modo: é substituir o contrato da etapa, transacionalmente e sem manter dois caminhos mutantes, preservando os 21 itens do menu, os entrypoints e os status públicos `0/1/2/3`.

**Ordem de preferência dos modos, decidida em I9.12 e não negociável por conveniência:** memória normal com THP oportunístico é o baseline; hugetlb de 2 MiB em runtime é o modo hugetlb preferencial; 1 GiB em runtime é `best-effort` fail-closed, porque cada página exige 1 GiB fisicamente contíguo e 22 delas não podem ser garantidas após uptime, pressão ou fragmentação; 1 GiB no boot foi **removido do projeto em I9.12-D8** (05/09/2026): ele era declaradamente não retornável, e manter a opção ao lado do contrato novo deixava dois caminhos mutantes em produção. Os três modos que restam (`normal`, `hugetlb-2m`, `hugetlb-1g`) são todos retornáveis. Nenhum fallback silencioso entre modos, e nenhum padrão silencioso: política não decidida faz as etapas 14 e 17 recusarem aplicar.
...
**Pendências de I9.12 em 05/09/2026, depois da retomada.** As três pendências de executor foram fechadas e o Gate I9 foi reexecutado com o veredito observado; o que resta não é código:

1. ~~Teste dirigido da etapa 18~~ **FEITO** em 05/09/2026: `tests/test-i912-etapas-17-18.sh` cobre os quatro casos pedidos (ausência das três chaves com persistência comprovada devolvendo rc 0 e o texto do perfil retornável; chaves presentes e exatas devolvendo rc 1 com o aviso de não retornável; `confirmar_digitando` recusada sem produzir efeito nenhum; e a regressão da árvore anterior, que relatava a ausência como divergência), mais a prova de que a confirmação digitada vem ANTES de qualquer escrita de boot.
2. ~~Pergunta de `MEMORIA_MODO` na etapa 3~~ **FEITA** em 05/09/2026 (I9.12-D12): três opções na ordem `normal`, `hugetlb-2m`, `hugetlb-1g`, sem pré-seleção, repergunta em `--redetectar`, gravação pelo mesmo `salvar_conf` atômico. O `--verificar` da etapa 3 passou a exigir a chave, e `HUGEPAGES_1G` saiu do fluxo.
3. ~~Gate I9 reexecutado com veredito observado~~ **FEITO** em 05/09/2026: `OK: Gate I9 concluído sem mascarar status`, rc 0 lido diretamente do runner. Log em `~/gate-tmp/gate-i9-20260905-0304.log`.
4. **Operador, neste host** (nada disto pode ser feito pelo executor: exige `sudo` interativo, `virsh define` real e a janela de GPU única, regras 12 e 13). Na ordem:
   1. `bash etapas/02-detectar-config.sh`: escolher a política de memória (o alvo combinado para este host é `hugetlb-2m`, I9.12-D14). A mesma execução remove `HUGEPAGES_1G=22` do conf pela migração pré-parser. Conferir com `grep -E 'MEMORIA_MODO|HUGEPAGES' passthrough.conf`.
   2. `bash etapas/52-cpu-pinning-hugepages.sh --verificar` e depois `bash etapas/52-cpu-pinning-hugepages.sh`: define o XML com pinning e `<page size="2048" unit="KiB"/>`, sem reboot. Conferir com `virsh --connect qemu:///system dumpxml vwin11 | grep -A3 memoryBacking`.
   3. `bash etapas/50-hooks-gpu-hd1.sh`: instala os hooks com `mem_adquirir`/`mem_devolver` assados com `MEMORIA_MODO=hugetlb-2m`. Conferir com `grep -n 'MEMORIA_MODO=\|MEM_PAGES_NEEDED=' /etc/libvirt/hooks/qemu.d/vwin11/prepare/begin/01-gpu-preflight.sh` (esperado 11264 páginas de 2048 kB para 22528 MiB).
   4. Anotar o baseline: `grep -E 'HugePages_|Hugetlb|MemAvailable' /proc/meminfo`.
   5. `[H]` Iniciar a VM uma vez, em janela escolhida (GPU única derruba a sessão gráfica): pelo SSH, ou depois do desligamento, ler as linhas `mem:` de `/var/log/vm-passthrough/hooks.log` e `sudo cat /var/lib/vm-passthrough/vwin11.memoria` (estado `RELEASED` ou arquivo ausente).
   6. `[H]` Com a VM parada, repetir o passo 4: `HugePages_Total=0`, `Hugetlb=0` e `MemAvailable` na ordem de 28 GB. **Este é o aceite do perfil retornável neste host**; registrar na seção 12 com os números.
   7. Se o start for recusado por aquisição (`mem ERRO` no log), o desktop continua de pé por desenho; o diagnóstico está no log e o modo pode voltar a `normal` pela etapa 3, seguida das etapas 17 e 14.
...
6. Só então declarar `MEMORIA_MODO` e habilitar o ciclo de vida dinâmico. **Este passo é do OPERADOR e continua devendo em 05/09/2026**: o código que ele exige ficou pronto na retomada (a etapa 3 pergunta, a etapa 17 aplica o XML, a etapa 14 assa a política nos hooks), mas executar as três exige `sudo` interativo e `virsh define` real. A sequência exata está no item 4 das pendências de I9.12, acima, transcrita para não depender de nenhum arquivo fora deste plano.
...
**Resultado do Gate I9 (2026-09-05): APROVADO, com veredito OBSERVADO.** Executado na raiz física do checkout em `e47a5bc`, com `GATE_FASE=I9`, `TMPDIR` e `MUTATOR_HARNESS_TMP_PARENT` em `$HOME/gate-tmp` (fora de `/tmp`, pela lição de 27/08/2026). Duração de cerca de 1 h 5 min. Última linha do log: `OK: Gate I9 concluído sem mascarar status`, com `rc=0` lido do runner, não de wrapper. Cobriu: manifesto nominal com **162** caminhos; envelope I1 com 30 mutadores diretos e 23 seleções de menu em 6 perfis, duas vezes; campanha I0 `full` integral sem skips; suíte histórica completa, incluindo `tests/test-python-core.sh` (**1158** casos unittest) e a suíte dirigida nova `tests/test-i912-etapas-17-18.sh`; `bash -n`, `compileall` com cache externo e nenhum bytecode residual; fronteira Python; whitespace de working tree, index e untracked (0 arquivos untracked). Nenhum arquivo do checkout foi editado enquanto o gate rodava (regra 21). Log em `~/gate-tmp/gate-i9-20260905-0304.log`.
...
| REQ-VM-RESOURCE-LIFECYCLE | recursos dedicados voltam ao host com a VM parada | I9.12 (código)/I13 | plano puro em `tests/python/test_resources.py`; XML por tamanho de página em `tests/python/test_domain_xml.py`; hooks com sysfs simulado e oráculo diferencial em `tests/test-i912-memoria-hooks.sh` (89 casos); contrato das etapas 17 e 18 em `tests/test-i912-etapas-17-18.sh` (26 casos); ordem do apply e ausência de escrita de boot em `tests/test-cpu-hugepages.sh` | baseline de RAM/CPU restaurado em hardware, I13 |
...
| Nota de método | 2026-09-05 | `af07725` + working tree | nenhum arquivo rastreado | o plano de execução da retomada viveu como `PLANO-EXECUCAO-I9.12-OPUS.md` na raiz, untracked e excluído por `.git/info/exclude`, e foi APAGADO ao final sem nunca ser commitado | registrado | arquivo temporário não é fonte de verdade: as sete decisões que ele carregava (I9.12-D8 a D14) foram gravadas na fase I9 ANTES da primeira linha de código, exatamente pela lição de 02/09/2026, quando a infraestrutura de I9B se perdeu com o diretório da sessão | fase I9, bloco de decisões de 05/09/2026 | nada pendente |
...
| I9.12 (retomada) | 2026-09-05 | `af07725` a `e47a5bc` (8 commits) | **núcleo**: `libexec/passthrough_core/{resources,config,cpu,domain_xml}.py`; **shell**: `lib/shell/{config,probes,libvirt}.sh`, `etapas/{02,50,52}`, `menu.sh`, `util/diagnostico.sh`; **testes**: novo `tests/test-i912-etapas-17-18.sh`, alterados `tests/python/{test_resources,test_config,test_cpu,test_domain_xml}.py`, `tests/test-{cpu-hugepages,i0-characterization,i3-domain-transactions,i4-config,i5-cpu-boot,i9-hooks-isolados,i912-memoria-hooks,inventario-redetectar,gpu-udev-loop,runtime-lifecycles}.sh`, `tests/lib/{i1-guard,mutator}-harness.sh`, `tests/i0/traceability.tsv`, `tests/manifests/i9-files.txt`; **docs**: `passthrough.conf.example`, `README.md`, `Guia-QEMU-Passthrough.md`, `troubleshooting.md`, `.kiro/steering/{product,tech}.md`, `.kiro/specs/boot-initramfs-multidistro/*.md`, este plano | baseline VERDE antes da primeira mudança (1146 casos Python, 87 casos de memória, manifesto com 161 caminhos). Ao final: `run_tests.py` **1158 casos**; `test-i912-memoria-hooks.sh` **89 casos**; `test-i912-etapas-17-18.sh` **26 casos** (novo); as 25 suítes `tests/test-*.sh` do laço rc 0; `test-i1-safety-envelope.sh` 30+23 em 6 perfis duas vezes; `test-atualizar-host-validation.sh` 29 cenários; `check-phase-manifest.sh I9` **162** caminhos; `check-python-boundary.py` e `check-waivers-matrix.py` rc 0. **Gate I9 canônico: `OK: Gate I9 concluído sem mascarar status`, rc 0, veredito OBSERVADO** | **APROVADO** | nada de I9.12 depende mais do executor. O que resta é do OPERADOR neste host (escolher `MEMORIA_MODO` na etapa 3, reexecutar as etapas 17 e 14, iniciar a VM uma vez e provar `HugePages_Total=0`/`Hugetlb=0` com ela parada) e o aceite em hardware de I13. O passo 3 da sequência de migração (VM com memória comum) continua devendo | fase I9: tabela de provas, decisões I9.12-D8 a D14, parágrafo do Gate I9; log em `~/gate-tmp/gate-i9-20260905-0304.log` | fase ativa passa a ser **I9B**: reconstruir a infraestrutura de i18n na árvore, começando por I9B.1 |
...
- [~] REQ-VM-RESOURCE-LIFECYCLE: recursos dedicados voltam ao host com a VM parada (I9.12: migração, núcleo, hooks, contratos das etapas 17 e 18 e, na retomada de 05/09/2026, a remoção do perfil de reserva no boot, a pergunta de `MEMORIA_MODO` na etapa 3 e o teste dirigido das etapas 17 e 18 em `tests/test-i912-etapas-17-18.sh`. Falta o aceite I13, que é hardware real).
