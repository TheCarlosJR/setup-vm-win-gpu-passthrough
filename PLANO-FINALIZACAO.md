...
| 9 | I7 | Rede transacional e planner backend-neutral | `EM ANDAMENTO` 2026-08-26 (`I7.1` a `I7.4` concluídas) | `network.py`, REQ-NET-TX |
...
- [~] **I7.5:** Bash captura, confirma, revalida fingerprints, aplica, arma traps, verifica, commita e restaura; Python só verifica snapshots. **Parcial em 2026-08-26.** Entregue: o rollback deixou de confiar em código de retorno e passa a reler e comparar semanticamente cada recurso restaurado (XML da VM por `xml_dominio_equivalente`, XML de rede por fingerprint canônico do core, artefato de configuração e `passthrough.conf` por conteúdo, estado ativo pelo master do uplink), fechando `D-NET-ROLLBACK-DIVERGE`; a confirmação passou a acontecer **antes da primeira mutação** nos dois modos, e `exigir_comando netplan` e a recusa de Wi-Fi subiram junto para não recusarem depois de gravar configuração; `netmask_para_prefixo` (código morto sem chamador) foi removido. **Pendente:** captura do snapshot completo, revalidação de fingerprints (`D-NET-CONCURRENCY`) e execução do plano pela etapa, todos bloqueados pela ausência de adaptador de transporte descrita na linha I7.5 da seção 12.
...
- [~] **I7.7:** manter a intenção independente de Netplan; não criar parser YAML manual. Providers futuros escolhem backend. **Núcleo concluído em 2026-08-26:** a intenção modela o artefato como `{scope, identifier, content, mode}` com identificador lógico (o schema recusa `/`) mais parâmetros declarativos; o core nunca gera nem interpreta o conteúdo e nenhum parser YAML foi criado. Um teste estático reprova qualquer token de ferramenta (`netplan`, `virsh`, `nmcli`, `networkd`, `wicked`, `/etc/netplan`) em qualquer valor do plano; a tradução verbo→comando vive só na tabela de projeção do teste. **Pendente:** `etapas/60-rede-bridge.sh` ainda chama Netplan inline em vez de por mapa de provider; essa fiação é de I7.5.
...
| I7 | | | | | em andamento (`I7.1`–`I7.4` concluídas; `I7.5` parcial com transporte pronto; `I7.7` com núcleo pronto) | Gate I7 exige concluir I7.5, mais I7.6 e revisão específica de rollback | | fiar a etapa 19 ao plano |
