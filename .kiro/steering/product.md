...
- I9 (modularização Bash e requisitos P1) foi **CONCLUÍDA em 05/09/2026**, com o Gate I9 reexecutado e o veredito `OK: Gate I9 concluído sem mascarar status` OBSERVADO (rc 0, log em `~/gate-tmp/gate-i9-20260905-0304.log`). Ela estivera reaberta por I9.12, REQ-VM-RESOURCE-LIFECYCLE: recursos dedicados à VM voltam ao host quando ela para. Feito: migração do host, núcleo `resources.py`, hooks com aquisição e devolução de memória, contratos das etapas 17 e 18 substituídos e, em 05/09/2026, a retomada que fechou o requisito no código: o perfil `hugetlb-1g-boot` foi REMOVIDO do catálogo (I9.12-D8), `HUGEPAGES_1G` virou chave depreciada (D9), a etapa 17 aplica somente XML e nunca mais grava parâmetro de boot (D10), o XML declara a página pelo modo (D11), a etapa 3 pergunta `MEMORIA_MODO` (D12) e a etapa 14 recusa assar política não decidida (D13). Pendente de I9.12: apenas a sequência do operador neste host (escolher `MEMORIA_MODO` na etapa 3, reexecutar as etapas 17 e 14, iniciar a VM uma vez e provar `HugePages_Total=0`/`Hugetlb=0` com ela parada) e a qualificação em hardware (I13).
- I9B (internacionalização en/pt-BR/es) é a **fase ativa** desde 05/09/2026. A infraestrutura construída em 02/09/2026 foi PERDIDA sem commit; nenhum arquivo dela existe na árvore (`lang/`, `lib/shell/i18n.sh`, `messages.py`, `tests/check-i18n-catalogs.py`). As decisões e medições recuperadas estão na fase I9B do plano.
