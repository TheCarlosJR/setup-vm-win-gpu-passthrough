...
verificar() {
    ...
    if vm_existe "$VM_NAME"; then
        ...
        if [ -n "${HD1_BY_ID_PATH:-}" ] && grep -qF "$HD1_BY_ID_PATH" <<< "$xml"; then
            v_ok "HD1 anexado ao XML."
        else
            v_falta "HD1 ainda não anexado ao XML."
        fi
    else
...
echo "[hook] Vinculando GPU (${GPU_PCI}) e audio (${GPU_AUDIO_PCI}) ao vfio-pci..."
...
GPU_IDS="${GPU_VENDOR_DEVICE_ID},${GPU_AUDIO_VENDOR_DEVICE_ID}"
...
ok "Hooks gerados com: GPU=$GPU_PCI_ID audio=$GPU_AUDIO_PCI_ID ids=$GPU_IDS dm=$DM_SERVICE"
...
# ----------------------------------------------------------------------------
# 4. Anexar o HD1 físico
# ----------------------------------------------------------------------------
titulo "4/4 HD1 físico no XML da VM"
if $VIRSH dumpxml --inactive "$VM_NAME" | grep -qF "$HD1_BY_ID_PATH"; then
    info "HD1 já anexado; pulando."
else
    [ -e "$HD1_BY_ID_PATH" ] || falhar "HD1 não encontrado: $HD1_BY_ID_PATH (disco conectado?)"
    ...
    if mount | grep -q "^$ALVO"; then
        falhar "O HD1 ($ALVO) está MONTADO no host. Nunca monte HD1 no host com a VM em uso. Desmonte antes."
    fi
...
XML
    $VIRSH attach-device "$VM_NAME" "$TMP" --config
    rm -f "$TMP"
    ok "HD1 anexado como vdb (VirtIO, raw, cache=none)."
fi
